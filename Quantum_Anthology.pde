...
// --- AYARLAR ---
int FADE_DURATION = 50;   // Geçiş hızı (Daha yumuşak ama hızlı)
...
void setup() {
  ...
  // Font Ayarları (Sistem fontu kullanıyoruz, istersen createFont ile özel font yükleyebilirsin)
  titleFont = createFont("Arial Bold", 24);
  bodyFont = createFont("Arial", 14);
  
  scenes = new Scene[5];
  
  // 1. ESER: IS THIS QUANTUM COMPUTING (Eski Sicim)
  scenes[0] = new SceneSicim();
  scenes[0].duration = 200; // ~3.3 Saniye
  ...
  // 2. ESER: HYPNOTIC GLUONS (Eski Gluon - EN KISA SÜRE)
  scenes[1] = new SceneGluon();
  scenes[1].duration = 90; // ~1.5 Saniye (Çok hızlı geçecek)
  ...
  scenes[2].duration = 180; // ~3 Saniye
  ...
  // 4. ESER: ORBIT (Eski Kütleçekim)
  scenes[3] = new SceneGravity();
  scenes[3].duration = 180; // ~3 Saniye
  ...
  scenes[4].duration = 200; // ~3.3 Saniye
...
void drawUI() {
  // Mevcut sahnenin bilgilerini al (Geçiş sırasında karışmaması için currentScene kullanılır)
  Scene s = scenes[currentSceneIndex];
  
  // Alt Panel Arkaplanı (Okunabilirlik için)
  noStroke();
  ...
  textFont(bodyFont);
  textSize(13);
  textLeading(18); // Satır aralığı
  // Yazıyı kutu içine sığdır (x, y, genişlik, yükseklik)
...
class SceneSicim extends Scene {
  ...
  void setupScene() { time = 0; }
  void update() { time += 0.02; }
...
class SceneHypercube extends Scene {
  ...
  void drawCube(float s) {
    ...
    v[0]=new PVector(-s/2,-s/2,-s/2); v[1]=new PVector(s/2,-s/2,-s/2);
    v[2]=new PVector(s/2,s/2,-s/2); v[3]=new PVector(-s/2,s/2,-s/2);
    v[4]=new PVector(-s/2,-s/2,s/2); v[5]=new PVector(s/2,-s/2,s/2);
    v[6]=new PVector(s/2,s/2,s/2); v[7]=new PVector(-s/2,s/2,s/2);
    for (int i = 0; i < 4; i++) {
      int n = (i + 1) % 4;
...
class SceneGravity extends Scene {
  ...
  void setupScene() { center = new PVector(width/2, height/2); }
  void update() {}
